...
filegroup = "dmmr"
  data <- read.table("data/species.txt")
  data <- read.csv("data/metabolites.csv", row.names = 1)
  filename = glue("Result/{filegroup}/")
  # metadata <- read.csv("metadata.csv", row.names = 1)
  metadata <- read.csv(glue("data/meta_{filegroup}_full.csv"), row.names = 1)
  # metadata <- metadata[colnames(mat), ]
  table(colnames(data) %in% rownames(metadata))
  data <- data[, colnames(data) %in% rownames(metadata)]
  
  # data <- read.table("adj_species.txt")
  # metadata <- read.csv("adj_meta.csv", row.names = 1)
  ...
  data_transform = 'auto'
  group_variable = filegroup
  color_variable = filegroup
  color_variable_order = c("CTRL", "pMMR", "dMMR")
...
{  # Keep same columns of data with rows of metadata
  ...
  p
  pdf(paste(filename, "_PCoA", ".pdf", sep = ""), width = 6, height = 8)
  p + theme_bw() +
    theme(
      legend.position = "right",
